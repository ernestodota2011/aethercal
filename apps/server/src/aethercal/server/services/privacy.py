...
.. rubric:: The four structural locks

A purge is written once, against the schema of that day, and then rots in silence. Four
...
* :data:`RETAINED_OUTBOX_COLUMNS` must cover every column on ``outbox``, because a RETAINED row
  keeps all of them — ==the question three rounds spent on ``payload`` never asked==. It is derived
  from ``Outbox.__table__.columns``, so a column added tomorrow fails the suite until somebody
  writes down why an erasure may keep it, and what makes that true.
"""
...
RETAINED_OUTBOX_COLUMNS: dict[str, str] = {
    "id": "our UUID, generated by us. It is also what the anomaly line names, precisely because it "
    "identifies the ROW and nobody else.",
    "tenant_id": "our UUID for the BUSINESS. The erasure is scoped to one tenant; the tenant is "
    "not the guest.",
    "created_at": "when WE queued the intent. A fact about our own machinery.",
    "booking_id": "our UUID, pointing at a row the erasure REDACTS in place rather than deletes "
    "(the appointment happened; the slot was genuinely taken). The person is erased there.",
    "effect": "one of OUR words — an OutboxEffect value, and nothing else can reach this column "
    "because `enqueue_effect` is the only place an Outbox row is constructed (locked by "
    "`test_the_funnel_is_the_only_place_an_outbox_row_is_constructed`). That lock is what lets the "
    "anomaly line quote it.",
    "dedupe_key": "REBUILDABLE from the retained payload, and refused at the funnel when it is not "
    "(`RETAINED_DEDUPE_KEY`). It cannot be redacted — `we_queued_this_refund` matches on it, so it "
    "IS the r8 discriminator — hence it must be PII-free by construction instead.",
    "payload": "rebuilt from `RETAINED_PAYLOAD_KEYS`; everything undeclared is dropped "
    "(`_retained_payload`).",
    "status": "one of OUR words (an OutboxStatus value); same funnel argument as `effect`.",
    "attempts": "a count of our own retries.",
    "last_attempt_at": "when WE last called a provider.",
    "next_retry_at": "when WE will try next.",
...
==The question three rounds of hardening ``payload`` never asked: what ELSE does the row carry?==
A retained row keeps all thirteen of these, and until this table existed only one of them had ever
been looked at. ``dedupe_key`` — free-form ``VARCHAR(128)``, built from whatever the caller passed —
rode along on every refund the erasure kept.

Asserted against ``Outbox.__table__.columns`` (``test_privacy``), so the classification is DERIVED
FROM THE MODEL rather than from somebody's memory of it — the same shape as
:data:`BOOKING_PII_COLUMNS`, which does this for ``bookings``. A column added tomorrow fails the
suite until somebody writes down why an erasure may keep it, and what makes that true."""
