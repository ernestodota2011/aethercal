...
async def test_an_unbranded_business_shows_the_operator_empty_boxes_and_its_real_name(
    ...
    view = await branding_view(_admin(sessionmaker), tenant_slug="sol")
...
async def test_saving_round_trips(sessionmaker: Sessionmaker) -> None:
    ...
    await update_branding_action(admin, tenant_slug="sol", form=SOL)
    view = await branding_view(admin, tenant_slug="sol")
...
async def test_the_operator_edits_only_their_own_business(sessionmaker: Sessionmaker) -> None:
    ...
    await update_branding_action(admin, tenant_slug="luna", form=LUNA)
    await update_branding_action(admin, tenant_slug="sol", form=SOL)

    sol = await branding_view(admin, tenant_slug="sol")
    luna = await branding_view(admin, tenant_slug="luna")
...
async def test_a_bad_value_is_refused_with_a_readable_message_and_nothing_is_written(
    ...
    with pytest.raises(AdminActionError) as caught:
        await update_branding_action(admin, tenant_slug="sol", form=BrandingForm(**bad))
