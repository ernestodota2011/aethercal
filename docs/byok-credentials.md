...
| **the refund is idempotent, and a DEAD refund is retryable** | phase B uses production's own `refund_idempotency_key`, so a retry gets the same refund and never a second. A refund that ends **terminally failed** (`failed`/`canceled` — no money moved) opens a **new generation** named by that failure's id: without it the provider replayed the dead refund for ever and ==the guest was never paid back while the retry reported success== |
