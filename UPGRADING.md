...
- **Inbound replies** ("1" confirms, "2" cancels, STOP/BAJA/… opts out) are processed, take effect
  on the booking and the opt-out list, and the first two are **acknowledged** — queued as an outbox
  intent, so the acknowledgement passes consent, the opt-out list, the caps and the ledger (and a
  retried webhook sends exactly one). The acknowledgement does **not** require the OTP seal: the
  guest is answering FROM that number, which is stronger evidence than the code. **An opt-out is
  never acknowledged** — the suppression is the answer.
