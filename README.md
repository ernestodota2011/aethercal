...
| Payments — Stripe and Mercado Pago checkout, unpaid holds that self-cancel, refunds and disputes, each business on its own (BYOK) account | Working in **provider test mode only**, now enforced **per gateway operation**: a live credential is refused while any operation of that provider's gateway has never been run against the real API. Today that is *every* operation of both — the Stripe adapter has only ever spoken to a stubbed transport, and no Mercado Pago account exists. The refusal is lifted by running the verification harness (`apps/server/tests/live/`) and recording what it observed, not by editing a flag. Partial refunds are not modelled. See [docs/byok-credentials.md](docs/byok-credentials.md) |
